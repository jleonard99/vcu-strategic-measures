...
if(!exists("build.file")) source( 'enrl00-EG-201710.Rparams' ) 
source( 'mylibs.R' )
...
for (param in c("unit.abbr","unit.longname","unit.shortname","unit.type","unit.parent.longname")){
...
\usepackage{\Sexpr{orgtype}-header}
...
tableDump(tt,sort=F,commas=T,cleanNames=T,rename=F,
  ...
  notes ="Source: ODSMGR.VCU\\_SUMMARY\\_RESIDENCY (Census II) with local recodes"
)

@

\chapter{Enrollment trends by program}

<<>>=
# construct long and short versions. Combine TIDYR code (for plotting) with RESHAPE2 code (for reporting)

tt_long = (molten.enrl0
 %>% select( fct_academic_period_desc, fct_hybrid_program_desc1, fct_hybrid_program1, moe_students_enrolled )
 %>% group_by( "VCU",fct_academic_period_desc, fct_hybrid_program_desc1,fct_hybrid_program1 )
...
tt_wide = dcast(
  molten.enrl0,
  fct_college1+fct_hybrid_program_desc1+fct_hybrid_program1~fct_academic_period_desc,
  fun.aggregate=function(x){sum(x,na.rm=TRUE)},
  value.var="moe_students_enrolled",
  margins="fct_college1"
)
tt_wide = tt_wide[,2:length(names(tt_wide))]
names(tt_wide)[1:2] = c("Program","Abbr")
...
maxy = ceiling(max(gg_long$moe_stu_enrl)/100)*100
maxy = 300

g = (ggplot(gg_long,aes(x=fct_academic_period_desc,y=moe_stu_enrl,group=fct_hybrid_program1))
     + geom_line()
     + geom_point(size=0.5)
     + facet_wrap( ~fct_hybrid_program_desc1,scales="free_y")
     + geom_hline(yintercept=0, alpha=0)
     + geom_hline(yintercept=c(maxy),alpha=0)
